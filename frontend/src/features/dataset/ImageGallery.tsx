...
export default function ImageGallery() {
  ...
  const importZipDataset = async (files: FileList | null) => {
    if (!files || files.length === 0) return
    const file = files[0]
    if (!file.name.toLowerCase().endsWith('.zip')) {
      alert('Please select a valid ZIP file dataset.')
      return
    }
    
    setUploading(true)
    setUploadText(`Importing pre-annotated dataset zip: ${file.name}...`)
    setUploadPct(30)
    ...
    try {
      setUploadPct(60)
      const res = await api.post(`/projects/${projectId}/import-zip-dataset`, fd, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      })
      setUploadPct(100)
      setUploadText(res.data.message || 'Dataset imported successfully!')
    } catch (err: any) {
      setUploadPct(100)
      const errMsg = err?.response?.data?.detail || 'Import failed. Ensure the ZIP contains images and optionally a COCO JSON file.'
      setUploadText(`Error: ${errMsg}`)
    } finally {
      setTimeout(async () => {
        setUploading(false)
        setUploadText('')
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 48px)',
      ...
      <header className="gallery-header" style={{ display: 'flex', alignItems: 'center', gap: 0, flexShrink: 0,
        height: 48, background: 'var(--surface)', borderBottom: '1px solid hsl(var(--border))',
        padding: '0 16px' }}>
        ...
        <div className="gallery-actions" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <button onClick={() => zipInputRef.current?.click()}
            style={{ display: 'flex', alignItems: 'center', gap: 5, height: 30,
              padding: '0 10px', border: '1px solid hsl(var(--border))', borderRadius: 3,
              background: 'var(--surface-2)', color: 'var(--text)', cursor: 'pointer', fontSize: 11 }}
            title="Import pre-annotated dataset archive (ZIP format with COCO annotations)">
            <FileArchive size={13} /> <span className="btn-txt">Import Zip</span>
          </button>
      ...
      <input ref={zipInputRef} type="file" accept=".zip" style={{ display: 'none' }}
        onChange={e => importZipDataset(e.target.files)} />
