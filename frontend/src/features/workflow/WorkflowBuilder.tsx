...
export default function WorkflowBuilder() {
  ...
  useEffect(() => {
    ...
    api.get(`/projects/${projectId}`)
      ...
      .catch(() => setProject({ id: Number(projectId) || 1, name: "Project", task_type: "image_classification", classes: [] }));
  ...
  const runTrainerJob = async (trainerId: string) => {
    ...
    try {
      ...
      const payload = {
        ...
        augmentation_types: config.augmentation_enabled ? (config.augmentation_types || ["horizontal_flip", "vertical_flip", "random_rotation", "color_jitter"]) : [],
        early_stopping: !!config.early_stopping,
        patience: config.early_stopping ? (Number(config.patience) || 3) : undefined
      };
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="flex items-center gap-2 mt-2">
              <input
                ...
                checked={node.config.augmentation_enabled !== false}
                onChange={(e) => {
                  handleConfigChange("augmentation_enabled", e.target.checked);
                  if (e.target.checked && !node.config.augmentation_types) {
                    handleConfigChange("augmentation_types", ["horizontal_flip", "vertical_flip", "random_rotation", "color_jitter"]);
                  }
                }}
                onBlur={() => handleSaveWorkflow()}
                className="w-4 h-4 rounded text-primary focus:ring-primary border-border bg-background"
              />
              <label htmlFor="augmentation_enabled" className="font-semibold cursor-pointer">Data Augmentation</label>
            </div>
            {node.config.augmentation_enabled !== false && (
              <div className="mt-2 pl-6 bg-muted/20 p-2 rounded-md border border-border/50">
                <span className="text-xs font-semibold block mb-2 text-muted-foreground uppercase tracking-wide">Augmentation Techniques</span>
                <div className="space-y-1.5">
                  {[
                    ...
                    const currentTypes = node.config.augmentation_types || ["horizontal_flip", "vertical_flip", "random_rotation", "color_jitter"];
                      ...
                      <label key={key} className="flex items-center gap-2 cursor-pointer text-xs">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            handleConfigChange("augmentation_types", nextTypes);
                          }}
                          onBlur={() => handleSaveWorkflow()}
                          className="w-3.5 h-3.5 rounded text-primary focus:ring-primary border-border bg-background"
                        />
                        <span>{label}</span>
                      </label>
                    );
            ...
            <div className="flex items-center gap-2 mt-2">
              <input
                type="checkbox"
                id="early_stopping"
                checked={node.config.early_stopping !== false}
                onChange={(e) => handleConfigChange("early_stopping", e.target.checked)}
                onBlur={() => handleSaveWorkflow()}
                className="w-4 h-4 rounded text-primary focus:ring-primary border-border bg-background"
              />
              <label htmlFor="early_stopping" className="font-semibold cursor-pointer">Early Stopping</label>
            </div>

            {node.config.early_stopping !== false && (
              <div className="mt-2 pl-6">
                <label className="block font-semibold mb-1">Patience (Epochs)</label>
                <input
                  type="number"
                  min={1}
                  max={100}
                  value={node.config.patience === undefined ? 3 : node.config.patience}
                  onChange={(e) => handleConfigChange("patience", parseInt(e.target.value) || 3)}
                  onBlur={() => handleSaveWorkflow()}
                  className="w-full bg-background border border-border text-foreground px-3 py-1.5 rounded-md text-xs"
                />
              </div>
