...
export default function Dashboard() {
  ...
  const filteredProjects = projects.filter((p) => {
    ...
    const matchesFilter = selectedTaskFilter === "all" || p.task_type === selectedTaskFilter;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 w-full bg-background text-foreground">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
        {[
          { id: 'all', label: 'All' },
          { id: 'image_classification', label: 'Classification' },
          { id: 'object_detection', label: 'Object Detection' },
          { id: 'image_segmentation', label: 'Segmentation' }
        ].map(filter => {
          const isActive = selectedTaskFilter === filter.id;
          return (
            <button
              key={filter.id}
              onClick={() => setSelectedTaskFilter(filter.id)}
              style={{
                padding: '6px 14px',
                borderRadius: 4,
                fontSize: 12,
